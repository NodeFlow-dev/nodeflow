...
func renderHAProxyConfig(routes []Route, facts NodeRenderFacts, allowEmpty bool) (HAProxyRenderResult, error) {
	...
	// v21 differs from v20 only by these node facts; a node without them
	// renders byte-identical v20 output, header included.
	if result.PipeSize == defaultPipeSize && autoTables == 0 {
	...
	b.WriteString("    timeout client 15m\n")
	b.WriteString("    timeout server 15m\n")
	if hasDynamicDNS {
		b.WriteString("\nresolvers nf_dns\n")
