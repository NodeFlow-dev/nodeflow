...
- **«Логи соединений HAProxy»** — переключатель в настройках ноды (`haproxy_logs`, в `nodes.metadata`, по умолчанию включено, конфиг байт в байт прежний). Выключено: без `log` в `global`, `log global`/`option tcplog`/`option dontlognull` в `defaults` и `option tcplog` у frontend'ов; переключение публикует новую ревизию ноды.
- **Исправлено:** маршрут из одного сервера (или одного домена в DNS-пуле) не сохранялся с `sticky_mode must be source, source_table, leastconn or roundrobin` — одиночный путь теперь принимает `none`.
- **Исправлено:** редактор маршрутов пропускал значения, которые API отклонял (IPv4 с ведущими нулями, некорректные IPv6, хосты с числовой последней частью, подсети доверенных адресов `/0`, дубли IPv6 в весах IP, резервный DNS-пул вместе с другими резервными, имена серверов как у слотов DNS-пула, запрещённые при DNS-пуле ручные директивы, табуляция в имени).
- **Исправлено:** ручная директива 509–512 байт сохранялась и ломала рендер конфига ноды; имя ноды длиннее 200 символов давало 500 вместо 400; версия релиза Agent проверяется до загрузки файла.
- Контрактный тест редактор ↔ API на 4685 вариантах маршрутов.

## 2.0.0 — Готовые образы и расширенная балансировка

Panel и Node Agent — 2.0.0, HAProxy renderer v21. Первый стабильный релиз после 1.0.8; в него вошли все изменения предварительных сборок 1.1.x. Заметки к релизу с инструкцией по обновлению: [docs/releases/2.0.0.md](docs/releases/2.0.0.md).
...
- **Числовые поля «Распределения клиентов»** (migration 000053): `sticky_table_entries` принимает любой размер `<n>k` (1..10000) или `<n>m` (1..10) вместо пресетов 10k/100k/1m (пресеты рендерятся как раньше); `balance_random_draws` — 1..5 вместо 1..2; для `leastping` — стоимость отдельного IP DNS-пула (`ip_weights[].cost`).
...
- **Splice-буферы 1 МиБ, когда ядро позволяет.** Node Agent **1.1.2** при старте и в каждом heartbeat поднимает `fs.pipe-max-size` до 1048576 (никогда не понижает) и ставит `fs.pipe-user-pages-soft = 0`: пишет прямо в `/proc/sys/fs/*` (без `sysctl`), сохраняет `/etc/sysctl.d/90-nodeflow-pipes.conf`; `NODE_AGENT_TUNE_SYSCTL=off` — только отчёт. Heartbeat сообщает `metrics.kernel_pipes {pipe_max_size, pipe_user_pages_soft, tuned, managed}`. systemd-юнит агента открывает на запись только эти два файла `/proc/sys` и `/etc/sysctl.d` (`ProtectKernelTunables` остаётся); самообновление юнит не меняет, поэтому на нодах, установленных 1.0.x, нужен однократный drop-in `20-kernel-pipes.conf` (см. заметки к релизу). Панель рендерит `tune.pipesize 1048576` для ноды, где `tuned=true`, `pipe_max_size ≥ 1048576` и `pipe_user_pages_soft = 0`, иначе прежние 262144.
...
- **«Меньше соединений»: вес и толерантность.** Для `balance_algorithm=leastconn` новые клиенты делятся пропорционально весу сервера и отдельного IP. В редакторе у этого алгоритма только «Вес»: стоимость там была бы просто обратным весом, поэтому колонки «Стоимость» нет, а сохранённые ранее `cost` при открытии маршрута переводятся в веса с теми же пропорциями. API по-прежнему принимает `servers[].cost` и `ip_weights[].cost` для `leastconn` (доля = вес ÷ стоимость, панель рендерит статический `weight` = base/cost × K; без стоимости конфиг байт-в-байт прежний). Новое поле `balance_tolerance` (алиас `leastping_tolerance`, та же колонка, без миграции): серверы, чья нагрузка (соединения / вес) не больше минимума на столько процентов, считаются равными и делят новых клиентов по весу; при толерантности > 0 Node Agent каждые 5 с читает `scur` и выставляет runtime-веса (гистерезис ×1,5, только изменённые веса). Требует Agent ≥ 1.1.1 (`422 leastconn_tolerance_requires_agent_1_1_1`). Маршрут из одного домена в режиме DNS-пула сохраняет толерантность и алгоритм (остаётся пулом серверов, а не сворачивается в одиночный маршрут). Подсказки к «Вес» и «Стоимость» в таблицах серверов и отдельных IP.

### Из предварительных сборок 1.1.x

Версии Agent 1.1.x в кодах проверок (`…_requires_agent_1_1*`) — ступени предварительных сборок; Agent 2.0.0 удовлетворяет всем.
...
- Migration 000029 widens the match_mode CHECK constraints for `fallback`; publish path loads route_servers; servers-only payload accepted with optional position; compose passes PANEL_METRICS_*; metrics without explicit timestamps + node_name label.
