...
## 2.0.2 — Подсказки к весу и стоимости

- В таблице «Вес и стоимость отдельных IP» у DNS-пула появились подсказки к «Вес» и «Стоимость» и строка с формулой и примером под текущий алгоритм (для «Меньше соединений»: доля IP = вес ÷ стоимость).

## 2.0.1 — Толерантность на одном DNS-пуле

- Исправлено: маршрут с одним сервером-доменом в режиме DNS-пула терял «Толерантность» и алгоритм «Меньше соединений» при сохранении — панель сворачивала его в старый формат одиночного маршрута, где эти настройки не хранятся. Теперь такой маршрут остаётся пулом серверов, и толерантность сохраняется и применяется агентом. Маршруты без толерантности и с закреплением по хешу IP рендерятся как раньше, байт в байт.
- Агент для этого не меняется; номер 2.0.1 у Panel и Node Agent общий.
